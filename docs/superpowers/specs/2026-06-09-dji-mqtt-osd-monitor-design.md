...
│   ├── PublishPanel.h/cpp      # Topic下发面板（下部，预留）
│   ├── ConfigDialog.h/cpp      # MQTT连接配置对话框
│   └── TopicEditDialog.h/cpp   # Topic增删改对话框
└── resources/
    └── config.json             # 默认配置模板
```
...
│ ✈ 手飞X  │                                                            │
│ ✈ 手飞Y  │  ◆ 位置信息                                                │
│          │  经度:113.412  纬度:23.058  高度:120.5m                   │
│ 右键菜单 │                                                            │
│ 添加设备 │  ◆ 飞行数据                                                │
│ 删除设备 │  电量:85%  速度:12m/s  航向:270°                           │
│ 编辑Topic│  飞行时间:8m32s  距home:1.2km  信号:▇▇▇▇▇                │
│          │                                                            │
│ ~200px   │  ◆ 姿态                                                    │
...
│          ├───────────────────────────────────────────────────────────┤
│          │  区域: Topic 下发 (预留 v1.1)                               │
│          │  Target Topic: [thing/product/xxx/services        ▾]      │
│          │  ┌───────────────────────────────────────────────────────┐│
│          │  │ {                                                     ││
│          │  │   "tid": "...",                                       ││
│          │  │   "method": "xxx",                                    ││
...
**右侧面板三个区域（垂直排列，可通过分割器调整高度）：**

| 区域 | 说明 | v1.0 |
|------|------|------|
| OSD 属性面板（上部） | 结构化展示 OSD 字段，按分组排列 | ✅ 实现 |
| 原始 JSON（中部） | 实时显示当前选中设备最新一条 OSD JSON，只读，支持复制 | ✅ 实现 |
| Topic 下发（下部） | 目标 topic 选择框 + JSON 编辑框 + 发送按钮，用于下发配置/指令 | 📋 预留界面，发送功能后续实现 |
...
| 删除设备 | 右键设备节点 → 删除 |
| 编辑 Topic | 右键设备节点 → 编辑 Topic（增删改） |
| 查看 OSD | 左侧单击设备节点，右侧面板刷新 |
| 查看原始 JSON | 选中设备后中部区域自动显示对应原始 JSON |
| 状态栏 | 实时显示连接状态和设备统计 |
