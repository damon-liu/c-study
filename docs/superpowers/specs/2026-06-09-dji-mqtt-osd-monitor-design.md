...
| 原始 JSON（中部） | 实时显示当前选中设备最新一条 OSD JSON，只读，右上角**一键复制按钮** | ✅ 实现 |
| Topic 下发（下部） | 目标 topic 选择框 + JSON 编辑框 + 发送按钮，**默认隐藏**，点击按钮展开 | 📋 仅 UI 占位，发送功能后续实现 |
...
| 查看原始 JSON | 选中设备后中部区域自动显示原始 JSON，点 📋 按钮复制到剪贴板 |
